...
export default function CharacterFormNew({
  errorParam,
  indexNumberParam,
}: {
  errorParam?: string
  indexNumberParam?: string
}) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [error, setError] = useState(errorParam)
  const [indexNumber, setIndexNumber] = useState(indexNumberParam || '')
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // 成功したら一覧ページにリダイレクト
      router.refresh()
      router.push('/admin/characters?success=created')
    } catch (err) {
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 ${
                  error === 'duplicate'
